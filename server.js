...
app.post('/api/auth/forgot-password', async (req, res) => {
  try {
    ...
    if (!phone && !email) {
      return res.status(400).json({ error: 'Teléfono o email requerido' });
    }
    ...
    console.log(`🔐 Código de recuperación generado para ${user.name} (${user.phone || user.email}): ${resetCode}`);

    res.json({ 
      message: 'Código de recuperación generado. Contacta al administrador para obtenerlo.',
      success: true,
      // En desarrollo, puedes mostrar el código. En producción, quitar esta línea:
      devCode: process.env.NODE_ENV === 'development' ? resetCode : undefined
    });
  } catch (error) {
    console.error('Error en forgot-password:', error);
    res.status(500).json({ error: 'Error al generar código', details: error.message });
  }
});
...
app.post('/api/auth/reset-password', async (req, res) => {
  try {
    ...
    if (!code || !newPassword) {
      return res.status(400).json({ error: 'Código y nueva contraseña requeridos' });
    }
...
// GET /api/admin/reset-codes - Admin ve códigos activos (útil para soporte)
app.get('/api/admin/reset-codes', authenticateToken, async (req, res) => {
  try {
    ...
    const usersWithCodes = await User.findAll({
      where: {
        resetCode: { [sequelize.Sequelize.Op.ne]: null },
        resetCodeExpiry: { [sequelize.Sequelize.Op.gt]: new Date() }
      },
      attributes: ['id', 'name', 'phone', 'email', 'resetCode', 'resetCodeExpiry', 'role']
    });

    res.json({ 
      codes: usersWithCodes.map(u => ({
        id: u.id,
        name: u.name,
        phone: u.phone,
        email: u.email,
        code: u.resetCode,
        expiresAt: u.resetCodeExpiry,
        role: u.role
      }))
    });
  } catch (error) {
